...
// 0 -> undetermined
// -1 -> wrong order
fn compare(left: Vec<String>, right: Vec<String>) -> i8 {
    for (i, item_a) in left.iter().enumerate() {
        ...
        if !type_is_arr(item_a) && !type_is_arr(item_b){
            if item_a.parse::<i32>().unwrap() < item_b.parse::<i32>().unwrap() { return 1 }
            if item_a.parse::<i32>().unwrap() > item_b.parse::<i32>().unwrap() { return -1 }
        }
         // Both are lists
...
fn get_indices() -> usize {
    ...
    for (i, pair) in data.iter().enumerate() {
        ...
        if compare(vec![l[0].to_string()], vec![l[1].to_string()]) > 0 { result += i + 1 }
    }
    result
}
